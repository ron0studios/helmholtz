...
glm::vec3 SceneSerializer::parseVec3(const std::string& str) {
    ...
    if (std::getline(iss, token, ',')) result.x = std::stof(token);
    if (std::getline(iss, token, ',')) result.y = std::stof(token);
    if (std::getline(iss, token, ',')) result.z = std::stof(token);
...
bool SceneSerializer::saveScene(const std::string& filepath,
    ...
    file << "showEmissionSource=" << (sceneData.showEmissionSource ? "true" : "false") << "\n";
    file << "showGeometryEdges=" << (sceneData.showGeometryEdges ? "true" : "false") << "\n\n";
...
bool SceneSerializer::loadScene(const std::string& filepath,
    ...
    while (std::getline(file, line)) {
        ...
        if (currentSection == "Camera") {
            if (key == "position") sceneData.cameraPosition = parseVec3(value);
            else if (key == "yaw") sceneData.cameraYaw = std::stof(value);
            else if (key == "pitch") sceneData.cameraPitch = std::stof(value);
        }
        else if (currentSection == "Grid") {
            if (key == "halfSize") sceneData.fdtdGridHalfSize = parseVec3(value);
            else if (key == "voxelSpacing") sceneData.voxelSpacing = std::stof(value);
            else if (key == "conductivity") sceneData.conductivity = std::stof(value);
        }
        else if (currentSection == "Visualization") {
            if (key == "gradientColorLow") sceneData.gradientColorLow = parseVec3(value);
            else if (key == "gradientColorHigh") sceneData.gradientColorHigh = parseVec3(value);
            else if (key == "showEmissionSource") sceneData.showEmissionSource = (value == "true");
            else if (key == "showGeometryEdges") sceneData.showGeometryEdges = (value == "true");
        }
        else if (currentSection == "Nodes") {
            if (key == "count") nodeCount = std::stoi(value);
        }
        else if (currentSection.find("Node") == 0) {
            // Parse node data
            if (key == "id") currentNode.id = std::stoi(value);
            else if (key == "name") currentNode.name = value;
            else if (key == "type") currentNode.type = static_cast<NodeType>(std::stoi(value));
            else if (key == "active") currentNode.active = (value == "true");
            else if (key == "position") currentNode.position = parseVec3(value);
            else if (key == "orientation") currentNode.orientation = parseVec3(value);
            else if (key == "frequency") currentNode.frequency = std::stof(value);
            else if (key == "color") currentNode.color = parseVec3(value);
            else if (key == "visible") {
    ...
    for (const auto& nodeData : tempNodes) {
        int newId = nodeManager->createNode(nodeData.position, nodeData.frequency, nodeData.type);
